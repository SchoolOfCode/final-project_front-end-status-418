...
*/
  function toggleState(currentDate) {
    ...
    let index = habitCopy.findIndex((element) => element.date === currentDate);
    ...
    if (index === -1) {
      ...
      switch (status) {
        case "incomplete":
          updatedState = changeStatus(habitItems, "complete", currentDate);
          break;
        case "complete":
        ...
        case "skip":
          ...
          break;
        default:
          updatedState = changeStatus(habitItems, "incomplete", currentDate);
          break;
      }
  ...
  // 	const res = await fetch(url + `/calendar?date=`)
  //   }
      ...
      <div className="habit-row">
        ...
        <div className="habit-item-container">
          {section.map((sectionday) => {
            ...
            let displayItem = habitItems.filter((item) => item.date === ymd);
